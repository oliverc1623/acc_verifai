...
def main() -> None:
    ...
    for update in range(1, num_updates + 1):
        ...
        for i in range(NUM_WORKERS):
            p = mp.Process(
                target=worker_fn,
                args=(i, STEPS_PER_WORKER, current_model_state_dict, data_queue, SEED + update * NUM_WORKERS),
            )  # Pass unique seed component
            p.start()
            processes.append(p)
        ...
        batch_actions = torch.tensor(np.concatenate(batch_actions_list), dtype=torch.int64).to(device)
